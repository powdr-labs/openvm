...
The set of VM extensions used by OpenVM is controlled by a configuration set in the `openvm.toml` file. Projects created by
`cargo openvm init` have the following minimal default configuration:

```toml [openvm.toml]
// [!include ~/snippets/examples_cli/openvm.toml]
```

To use additional [VM extensions](/book/acceleration-using-extensions/overview), you can specify and configure them in the `openvm.toml`
file. An example of an `openvm.toml` with all commonly used extensions is:
...
You still need the `openvm::entry!(main)` line.
