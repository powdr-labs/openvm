...
sw_declare! {
    ...
    P256Point { mod_type = P256Coord, a = P256_A, b = P256_B },
}
```
...
This creates `Bls12_381G1Affine` and `P256Point` structs which implement the `Group` and `WeierstrassPoint` traits. The underlying memory layout of the structs uses the memory layout of the `Bls12_381Fp` and `P256Coord` structs, respectively.
