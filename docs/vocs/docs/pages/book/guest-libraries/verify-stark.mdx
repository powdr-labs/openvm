...
Proofs are expected to be passed via the [hintable key-value store](/specs/openvm/isa#inputs-and-hints). Guests will query for proofs at the key `asm_filename || exe_commit_in_u32 || vm_commit_in_u32 || user_pvs`.
...
Hosts are expected to properly hint the [hintable key-value store](/specs/openvm/isa#inputs-and-hints) with the OpenVM STARK proofs. To that end, the library provides the following utilities:
