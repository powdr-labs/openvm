...
```bash
cargo +1.86 install --locked --git https://github.com/openvm-org/openvm.git --tag v1.4.0-rc.9 cargo-openvm
```
...
```bash
cargo +1.86 install --locked --git https://github.com/openvm-org/openvm.git --tag v1.4.0-rc.9 --features cuda cargo-openvm
```
...
You are now ready to [install](#installation-with-nvidia-gpu-support) the OpenVM CLI.
