...
```bash
cargo +1.90 install --locked --git https://github.com/openvm-org/openvm.git --tag v1.4.3 cargo-openvm
```
...
```bash
git clone --branch v1.4.3 --single-branch https://github.com/openvm-org/openvm.git
cd openvm
...
```bash
cargo +1.90 install --locked --git https://github.com/openvm-org/openvm.git --tag v1.4.3 --features cuda cargo-openvm
```
...
:::code-group
```bash [Without GPU]
cargo +nightly-2025-08-19 install --locked --git https://github.com/openvm-org/openvm.git --tag v1.4.3 --features tco cargo-openvm
```
```bash [Nvidia GPU]
cargo +nightly-2025-08-19 install --locked --git https://github.com/openvm-org/openvm.git --tag v1.4.3 --features tco,cuda cargo-openvm
```
:::
...
## Installation with Ahead of Time Compilation

On host machines with the `x86_64` architecture, the CLI may be installed with ahead of time (AOT) compilation of guest programs to host binaries. This feature provides the fastest execution performance. This feature may be enabled on top of the standard CPU installation as well as the Nvidia GPU installation. We provide installation instructions for both versions, assuming the prerequisites ([CPU](#installation-prerequisites), [GPU](#installation-prerequisites-cuda)) are already met.

**Caution**: AOT execution currently runs in _unprotected_ mode, and it should only be used on trusted guest programs.
...
:::code-group
```bash [Without GPU]
cargo +1.90 install --locked --git https://github.com/openvm-org/openvm.git --tag v1.4.3 --features aot cargo-openvm
```
```bash [Nvidia GPU]
cargo +1.90 install --locked --git https://github.com/openvm-org/openvm.git --tag v1.4.3 --features aot,cuda cargo-openvm
```
:::
...
:::code-group
```bash [Without GPU]
cargo +1.90 install --locked --force --path crates/cli --features aot
```
```bash [Nvidia GPU]
cargo +1.90 install --locked --force --path crates/cli --features aot,cuda
```
:::
