...
| hintrandom  | I   | 0001011     | 011    | 0x2       | Resets the hint stream to `4 * rd` random bytes from a fixed-seed RNG (`rand::rngs::StdRng`) on the host.                                                                                      |
...
| RISC-V Inst | FMT | opcode[6:0] | funct3 | funct7 | RISC-V description and notes                |
| ----------- | --- | ----------- | ------ | ------ | ------------------------------------------- |
| keccakf     | R   | 0001011     | 100    | 0x0    | Applies keccak-f[1600] permutation in-place on 200-byte state buffer at `rd`: `[rd:200]_2 = keccakf([rd:200]_2)`. `rs1` and `rs2` must be `x0`. |
| xorin       | R   | 0001011     | 100    | 0x1    | XORs `rs2` input bytes into state buffer: for `i` in `0..rs2`, `[rd+i]_2 ^= [rs1+i]_2`. `rs2` must be a positive multiple of 4 and at most 136. |

## SHA-2 Extension

| RISC-V Inst   | FMT | opcode[6:0] | funct3 | funct7 | RISC-V description and notes                       |
| ------------- | --- | ----------- | ------ | ------ | -------------------------------------------------- |
| sha256_update | R   | 0001011     | 100    | 0x2    | `[rd:32]_2 = compress256([rs1:32]_2, [rs2:64]_2)`  |
| sha512_update | R   | 0001011     | 100    | 0x3    | `[rd:64]_2 = compress512([rs1:64]_2, [rs2:128]_2)` |
