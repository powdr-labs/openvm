...
struct Rv32RdWriteAdapter {
    ...
    __device__ inline void fill_trace_row_new(RowSliceNew row, Rv32RdWriteAdapterRecord record) {
        COL_WRITE_VALUE_NEW(row, Rv32RdWriteAdapterCols, from_state.pc, record.from_pc);
        COL_WRITE_VALUE_NEW(row, Rv32RdWriteAdapterCols, from_state.timestamp, record.from_timestamp);
        COL_WRITE_VALUE_NEW(row, Rv32RdWriteAdapterCols, rd_ptr, record.rd_ptr);

        RowSliceNew aux_row = row.slice_from(COL_INDEX(Rv32RdWriteAdapterCols, rd_aux_cols));
        COL_WRITE_ARRAY_NEW(aux_row, MemoryWriteAuxCols, prev_data, record.rd_aux_record.prev_data);
        mem_helper.fill_new(
            aux_row.slice_from(COL_INDEX(MemoryWriteAuxCols, base)),
            record.rd_aux_record.prev_timestamp,
            record.from_timestamp
...
struct Rv32CondRdWriteAdapter {
    ...
    __device__ inline void fill_trace_row_new(RowSliceNew row, Rv32RdWriteAdapterRecord record) {
        bool do_write = (record.rd_ptr != UINT32_MAX);
        COL_WRITE_VALUE_NEW(row, Rv32CondRdWriteAdapterCols, needs_write, do_write);

        RowSliceNew inner = row.slice_from(COL_INDEX(Rv32CondRdWriteAdapterCols, inner));

        if (do_write) {
            Rv32RdWriteAdapter adapter(mem_helper.range_checker, timestamp_max_bits);
            adapter.fill_trace_row_new(inner, record);
        } else {
            inner.fill_zero_new(0, sizeof(Rv32RdWriteAdapterCols<uint8_t>));
            COL_WRITE_VALUE_NEW(
                inner, Rv32RdWriteAdapterCols, from_state.timestamp, record.from_timestamp
            );
            COL_WRITE_VALUE_NEW(inner, Rv32RdWriteAdapterCols, from_state.pc, record.from_pc);
        }
    }
};
