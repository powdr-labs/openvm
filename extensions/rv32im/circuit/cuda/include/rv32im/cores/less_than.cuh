...
template <size_t NUM_LIMBS> struct LessThanCore {
    ...
    __device__ void fill_trace_row_new(RowSliceNew row, LessThanCoreRecord<NUM_LIMBS> record) {
        constexpr uint8_t SLT = 0;
        ...
        COL_WRITE_VALUE_NEW(row, Cols, c_msb_f, c_msb_f);
        COL_WRITE_VALUE_NEW(row, Cols, diff_val, diff_val);
        if (!row.is_apc) {
            COL_WRITE_VALUE_NEW(row, Cols, opcode_slt_flag, is_slt);
            COL_WRITE_VALUE_NEW(row, Cols, opcode_sltu_flag, !is_slt);
        }
    }
};
