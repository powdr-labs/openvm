...
template <size_t NUM_LIMBS> struct BranchEqualCore {
    ...
    __device__ void fill_trace_row_new(RowSliceNew row, BranchEqualCoreRecord<NUM_LIMBS> rec) {
        size_t diff_idx = NUM_LIMBS;
        for (size_t i = 0; i < NUM_LIMBS; ++i) {
            if (rec.a[i] != rec.b[i]) {
                diff_idx = i;
        ...
        COL_WRITE_ARRAY_NEW(row, Cols, a, rec.a);
        COL_WRITE_ARRAY_NEW(row, Cols, b, rec.b);
        ...
        COL_WRITE_VALUE_NEW(row, Cols, imm, rec.imm);

        if (!row.is_apc) {
            COL_WRITE_VALUE_NEW(row, Cols, opcode_beq_flag, is_beq);
            COL_WRITE_VALUE_NEW(row, Cols, opcode_bne_flag, !is_beq);
        }
    }
};
