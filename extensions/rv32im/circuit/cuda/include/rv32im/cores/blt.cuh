...
template <size_t NUM_LIMBS> struct BranchLessThanCore {
    ...
    __device__ void fill_trace_row_new(RowSliceNew row, BranchLessThanCoreRecord<NUM_LIMBS> record) {
        static constexpr uint8_t BLT = 0;
        static constexpr uint8_t BLTU = 1;
        static constexpr uint8_t BGE = 2;
        static constexpr uint8_t BGEU = 3;
        ...
        COL_WRITE_ARRAY_NEW(row, Cols, a, record.a);
        COL_WRITE_ARRAY_NEW(row, Cols, b, record.b);
        COL_WRITE_ARRAY_NEW(row, Cols, diff_marker, diff_marker);

        COL_WRITE_VALUE_NEW(row, Cols, cmp_result, cmp_result);
        COL_WRITE_VALUE_NEW(row, Cols, imm, record.imm);
        if (!row.is_apc) {
            COL_WRITE_VALUE_NEW(row, Cols, opcode_blt_flag, record.local_opcode == BLT);
            COL_WRITE_VALUE_NEW(row, Cols, opcode_bltu_flag, record.local_opcode == BLTU);
            COL_WRITE_VALUE_NEW(row, Cols, opcode_bge_flag, record.local_opcode == BGE);
            COL_WRITE_VALUE_NEW(row, Cols, opcode_bgeu_flag, record.local_opcode == BGEU);
        }
        COL_WRITE_VALUE_NEW(row, Cols, a_msb_f, a_msb_f);
        COL_WRITE_VALUE_NEW(row, Cols, b_msb_f, b_msb_f);
        COL_WRITE_VALUE_NEW(row, Cols, cmp_lt, cmp_lt);
        COL_WRITE_VALUE_NEW(row, Cols, diff_val, diff_val);
    }
};