...
#pragma unroll
    ...
    __device__ void fill_trace_row_new(RowSliceNew row, DivRemCoreRecords<NUM_LIMBS> const &record) {
        DivRemOpcode opcode = static_cast<DivRemOpcode>(record.local_opcode);
        ...
        COL_WRITE_ARRAY_NEW(row, Cols, q, q);
        COL_WRITE_ARRAY_NEW(row, Cols, r, r);
        COL_WRITE_VALUE_NEW(row, Cols, zero_divisor, c_u32 == 0);
        COL_WRITE_VALUE_NEW(row, Cols, r_zero, r_zero);
        COL_WRITE_VALUE_NEW(row, Cols, b_sign, b_sign);
        COL_WRITE_VALUE_NEW(row, Cols, c_sign, c_sign);
        COL_WRITE_VALUE_NEW(row, Cols, q_sign, q_sign);
        COL_WRITE_VALUE_NEW(row, Cols, sign_xor, b_sign ^ c_sign);
...
#pragma unroll
        ...
        if (c_sum == 0) {
            COL_WRITE_VALUE_NEW(row, Cols, c_sum_inv, 0);
        } else {
            COL_WRITE_VALUE_NEW(row, Cols, c_sum_inv, inv(Fp(c_sum)));
        }

        if (r_sum == 0) {
            COL_WRITE_VALUE_NEW(row, Cols, r_sum_inv, 0);
        } else {
            COL_WRITE_VALUE_NEW(row, Cols, r_sum_inv, inv(Fp(r_sum)));
        }

        COL_WRITE_ARRAY_NEW(row, Cols, r_prime, r_prime);
#pragma unroll
        for (size_t i = 0; i < NUM_LIMBS; i++) {
            Fp r_inv = inv(Fp(r_prime[i]) - Fp(256));
            COL_WRITE_VALUE_NEW(row, Cols, r_inv[i], r_inv);
        }

        COL_FILL_ZERO_NEW(row, Cols, lt_marker);
        if (case_none && !r_zero) {
            uint32_t idx = NUM_LIMBS;
#pragma unroll
            for (int i = NUM_LIMBS - 1; i >= 0; i--) {
            ...
            COL_WRITE_VALUE_NEW(row, Cols, lt_marker[idx], 1);
            COL_WRITE_VALUE_NEW(row, Cols, lt_diff, val);
        } else {
            COL_WRITE_VALUE_NEW(row, Cols, lt_diff, 0);
        }

        if (!row.is_apc) {
            COL_WRITE_VALUE_NEW(row, Cols, opcode_div_flag, opcode == DIV);
            COL_WRITE_VALUE_NEW(row, Cols, opcode_divu_flag, opcode == DIVU);
            COL_WRITE_VALUE_NEW(row, Cols, opcode_rem_flag, opcode == REM);
            COL_WRITE_VALUE_NEW(row, Cols, opcode_remu_flag, opcode == REMU);
        }
...
__global__ void rv32_div_rem_tracegen(
    ...
    if (idx < d_records.len()) {
        ...
        core.fill_trace_row_new(row.slice_from(COL_INDEX(Rv32DivRemCols, core)), record.core);
    } else {
        if (!is_apc) {
