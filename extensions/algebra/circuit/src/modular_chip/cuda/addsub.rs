...
#[derive(new)]
pub struct ModularAddSubChipGpu<const BLOCKS: usize, const BLOCK_SIZE: usize> {
...
impl<const BLOCKS: usize, const BLOCK_SIZE: usize> Chip<DenseRecordArena, GpuBackend>
    for ModularAddSubChipGpu<BLOCKS, BLOCK_SIZE>
{
    fn generate_proving_ctx(&self, arena: DenseRecordArena) -> AirProvingContext<GpuBackend> {
        let range_bus = self.range_checker.cpu_chip.as_ref().unwrap().bus();
        let (expr, is_add_flag, is_sub_flag) = addsub_expr(self.config.clone(), range_bus);
