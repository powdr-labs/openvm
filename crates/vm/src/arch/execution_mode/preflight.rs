...
impl<RA: Arena> PreflightCtx<RA> {
    ...
    pub fn new_with_capacity(capacities: &[(usize, usize)], instret_end: Option<u64>, segment_idx: usize) -> Self {
        let arenas = capacities
            .iter()
            .map(|&(height, main_width)| RA::with_capacity(height, main_width, segment_idx))
            .collect();
