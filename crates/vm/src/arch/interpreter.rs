...
#[cfg(feature = "tco")]
use crate::arch::{ApcHandler, Handler};
use crate::{
    arch::{
        execution_mode::{
    ...
    ($span:literal, $interpreter:ident, $instret:ident, $pc:ident, $arg:ident, $exec_state:ident, $ctx:ident) => {{
        ...
        info_span!($span).in_scope(|| -> Result<(), ExecutionError> {
            ...
            #[cfg(feature = "tco")]
            {
                ...
                let handler = handler.handler(&$exec_state.vm_state);
    ...
    Ctx: ExecutionCtxTrait,
{
        ...
        E: Executor<F>,
    {
        ...
        let handlers = repeat_n(&None, base_index)
            ...
            .enumerate()
            .map(
                |(i, (inst_opt, pre_compute))| -> Result<ApcHandler<F, Ctx>, StaticProgramError> {
                    if let Some((inst, _)) = inst_opt {
                        ...
                        if get_system_opcode_handler::<F, Ctx>(inst, pre_compute).is_some() {
                            ...
                            let handler = ApcHandler {handler, apc: apc_handler_and_condition};
                            Ok(handler)
                        }
                    } else {
                        Ok((unreachable_tco_handler as Handler<_, _>).into())
                    }
                },
            )
    ...
    #[cfg(feature = "tco")]
    #[inline(always)]
    pub fn get_handler(&self, pc: u32) -> Option<&ApcHandler<F, Ctx>> {
        let pc_idx = get_pc_index(pc);
        self.handlers.get(pc_idx)
    }
                ...
                |(i, (inst_opt, pre_compute))| -> Result<ApcHandler<F, Ctx>, StaticProgramError> {
                    if let Some((inst, _)) = inst_opt {
                        ...
                        if get_system_opcode_handler::<F, Ctx>(inst, pre_compute).is_some() {
                            ...
                            let apc_handler_and_condition: Option<(Handler<F, Ctx>, ApcCondition)> = apc_inst.map(|(apc_inst, condition)| {
