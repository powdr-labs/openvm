...
// Helper function to check if a type is Vec<Vec<T>> or a type alias for it
fn is_vec_vec_of_generic(ty: &syn::Type, generic_type: &syn::TypeParam) -> bool {
    if let syn::Type::Path(type_path) = ty {
        // Check for direct Vec<Vec<T>>
        if type_path.path.segments.last().map(|s| &s.ident) == Some(&quote::format_ident!("Vec")) {
            if let syn::PathArguments::AngleBracketed(args) =
                &type_path.path.segments.last().unwrap().arguments
            {
                    ...
                    args.args.first()
                {
                    ...
                    return is_vec_of_generic(
                        &syn::Type::Path(inner_type_path.clone()),
                        generic_type,
                    );
                }
            }
        }
        // Check for Vecs<T> specifically by name and generic parameter count
        else if let Some(ident) = type_path.path.get_ident() {
            let name = ident.to_string();
            if name == "Vecs" {
                // Make sure it has exactly one generic parameter
                if let Some(last_segment) = type_path.path.segments.last() {
                    if let syn::PathArguments::AngleBracketed(args) = &last_segment.arguments {
                        if args.args.len() == 1 {
                            // Check if the generic parameter is T
                            if let Some(syn::GenericArgument::Type(syn::Type::Path(param_type))) =
                                args.args.first()
                            {
                                return param_type.path.is_ident(&generic_type.ident);
                            }
                        }
                    }
                }
